...
export type Case4RecipientBucket = 'to' | 'cc' | 'bcc';
export type Case4RecipientKind = 'internal' | 'unknown-domain';
export type Case4ArtifactId = 'suspicious-recipient' | 'audience-privacy' | 'payroll' | null;

export interface Case4Recipient {
  id: string;
  name: string;
  email: string;
  bucket: Case4RecipientBucket;
  kind: Case4RecipientKind;
  count: number;
  active: boolean;
  note: string;
}

export interface Case4Attachment {
  id: string;
  name: string;
...
export interface Case4EvaluationCheck {
...
export interface Case4Evaluation {
  level: 'perfect' | 'partial' | 'critical';
  title: string;
  summary: string;
  checks: Case4EvaluationCheck[];
...
export const createCase4Recipients = (): Case4Recipient[] => {
  const typoDomain = typoDomains[Math.floor(Math.random() * typoDomains.length)];
  const copiedRecipients: Case4Recipient[] = [
    {
      id: 'accounting',
      name: 'Contabilidad',
...
export const createCase4Attachments = (): Case4Attachment[] => shuffle([
  {
    id: 'payment-notice',
    name: 'aviso_pago.pdf',
    type: 'pdf',
...
export const getCase4CorrectedCount = (
...
export const evaluateCase4Draft = (
  recipients: Case4Recipient[],
  attachments: Case4Attachment[],
): Case4Evaluation => {
  ...
  const checks: Case4EvaluationCheck[] = [
    {
      id: 'recipient',
  ...
  const level: Case4Evaluation['level'] = correctedCount === 3 ? 'perfect' : correctedCount === 0 ? 'critical' : 'partial';
